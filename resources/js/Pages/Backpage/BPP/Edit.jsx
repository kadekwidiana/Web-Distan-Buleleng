...
export default function EditBPPPage() {
    ...
    const { bppById, districts, errors } = usePage().props;

    const { data, setData, post, progress, processing, recentlySuccessful } = useForm({
        district_id: bppById?.district_id,
        layer_group_id: 2,
        name: bppById?.name,
        location: locationInput,
        address: bppById?.address,
        phone_number: bppById?.phone_number,
        email: bppById?.email,
        leader: bppById?.leader,
        secretary: bppById?.secretary,
        treasurer: bppById?.treasurer,
        number_of_members: bppById?.number_of_members,
        since: bppById.since,
        status: bppById?.status,
        photos: JSON.parse(bppById?.photo) ?? [],
        description: bppById?.description,
    });
    ...
    const handleSubmit = (e) => {
        ...
        post(route('bpp-custom.update', { id: bppById?.id }), {
            data: formData,
            onSuccess: () => {
                Toast.fire({
                    icon: "success",
                ...
                <form onSubmit={handleSubmit} encType="multipart/form-data">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <div className="">
                                <MapsInputData isEdit={true} data={bppById} />
                            </div>
                        </div>
                    </div>
