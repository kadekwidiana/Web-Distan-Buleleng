...
export default function BPPPage() {
    const { bpps, districts, districtIdValue, searchValue } = usePage().props;
    const perpage = useRef(bpps.per_page);
    const [isLoading, setIsLoading] = useState(false);
    const [search, setSearch] = useState('');
    const district_id = useRef(bpps.district_id);
    ...
    const handleFilter = (e) => {
        district_id.current = e.target.value;
        getData();
    };
    ...
    const getData = (isSearchCleared = false) => {
        ...
        const params = pickBy({
            ...
            district_id: district_id.current ?? districtIdValue
        });
    ...
    const deleteData = async (id) => {
        await router.delete(route('bpp.destroy', { bpp: id }));
    };
    ...
    return (
        <BackpageLayout>
            <Head title="Balai Penyuluh Pertanian" />
            <div className="min-h-[84dvh] rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default sm:px-7.5 xl:pb-1">
                <div className="flex justify-between gap-2">
                    ...
                    <Button type="button" onClick={() => handleExportExcel(`BPP`, bpps.data)} className="bg-green-600 hover:bg-green-700 text-white text-nowrap px-3">
                        <i className="fa-solid fa-download"></i>
                        <span className='hidden sm:block'>Export Excel</span>
                    </Button>
                    <ButtonAdd href={'/bpp/create'} />
                </div>
                <div className="flex flex-col my-2">
                                ...
                                <Table.Row key={index} className="bg-white">
                                    ...
                                    <Table.Cell className='flex justify-center items-center gap-3'>
                                        ...
                                        <Dropdown placement="bottom-end" label="" dismissOnClick={false} renderTrigger={() =>
                                            ...
                                            <Dropdown.Item><Link href={route('bpp.edit', { bpp: bpp.id })}><i className="mr-2 fa-solid fa-pen-to-square text-blue-500 fa-xl"></i>Edit</Link></Dropdown.Item>
                                            <Dropdown.Item onClick={() => deteleDataConfirm(bpp.id)}><i className="mr-2 fa-solid fa-trash fa-xl text-red-500"></i>Hapus</Dropdown.Item>
                                        </Dropdown>
                                    </Table.Cell>
                                </Table.Row>
                            ))}
                    ...
                    {isLoading && <LoadData />}
                    {bpps.data.length <= 0 && !isLoading && <DataNotFound />}
                </div>
                {bpps.data.length > 0 && !isLoading &&
                    <div className="flex items-center justify-between m-2">
                        <div className="">
                            Showing {bpps.from} to {bpps.to} total{" "} {bpps.total}
                        </div>
                        <div className="flex items-center gap-2">
                            {bpps.links.map((link, index) => (
