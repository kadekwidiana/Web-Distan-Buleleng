...
import { CONFIRMATION_STATUSES, DATA_STATUSES, DATA_STATUSES_FOR_OPTIONS, GROUP_STATUSES } from '@/Constant/Status';
...
export default function StepOneCreateLandAgriculturePage() {
  ...
  const { landAgriculture, district, villages, poktans, subaks, typeLandAgricultures, owners, commodities, errors } = usePage().props;
  ...
  const [commoditiesCycleData, setCommoditiesCycleData] = useState(landAgriculture?.commodities_cycle ?? []);
  const { data, setData, post, progress, processing, recentlySuccessful } = useForm({
    village_id: landAgriculture?.village_id ?? '',
    poktan_id: landAgriculture?.poktan_id ?? '',
    subak_id: landAgriculture?.subak_id ?? '',
    type_land_agriculture_id: landAgriculture?.type_land_agriculture_id ?? '',
    owner_id: landAgriculture?.owner_id ?? '',
    cultivator_id: landAgriculture?.cultivator_id ?? '',
    status: landAgriculture?.status ?? 'ACTIVE',
  ...
  const handleSubmit = (e) => {
    ...
    post(route('landAgricultures.store.step.one', { districtId: district.id }), {
      data: formData
    });
  };
  ...
  return (
    <BackpageLayout>
      <Head title="Create Lahan Pertanian" />
      <div className="mb-2">
