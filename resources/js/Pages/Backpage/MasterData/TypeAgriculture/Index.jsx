...
import ModalInputTypeAgriculture from './ModalInput';

export default function MasterDataLayerGroupPage() {
    const { typeAgricultures, layerGroups, searchValue } = usePage().props;
    const perpage = useRef(typeAgricultures.per_page);
    const [isLoading, setIsLoading] = useState(false);
    const [search, setSearch] = useState('');
    const [openModal, setOpenModal] = useState(false);
    const [selectedTypeAgriculture, setSelectedTypeAgriculture] = useState(null);
    ...
    const deleteData = async (id) => {
        await router.delete(route('jenis-pertanian.destroy', { id: id }));
    }
    ...
    const handleEditClick = (typeAgriculture) => {
        setSelectedTypeAgriculture(typeAgriculture);
        setOpenModal(true);
        setIsUpdate(true);
    };
        ...
        <BackpageLayout>
            ...
            <div className="min-h-[84dvh] rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default sm:px-7.5 xl:pb-1">
                <div className="flex justify-between gap-2 sm:gap-10">
                    ...
                    <button onClick={() => {
                        setOpenModal(true);
                        setSelectedTypeAgriculture(null);
                        setIsUpdate(false);
                    ...
                    <ModalInputTypeAgriculture
                        openModal={openModal}
                        setOpenModal={setOpenModal}
                        typeAgricultureById={selectedTypeAgriculture}
                ...
                <div className="flex flex-col my-2">
                            ...
                            {!isLoading && typeAgricultures.data.map((typeAgriculture, index) => (
                                <Table.Row key={index} className="bg-white">
                                    ...
                                    <Table.Cell className='flex justify-center items-center gap-3'>
                                        <button
                                            onClick={() => handleEditClick(typeAgriculture)}
                                        >
                                            <i className="mr-2 fa-solid fa-pen-to-square text-blue-500 fa-xl"></i>
                                        </button>
                                        <button
                                            onClick={() => deteleDataConfirm(typeAgriculture.id)}
                                        >
                    ...
                    {isLoading && <LoadData />}
                    {typeAgricultures.data.length <= 0 && !isLoading && <DataNotFound />}
                </div>
