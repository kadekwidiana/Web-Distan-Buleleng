...
export default function MasterDataLayerGroupPage() {
    const { typeLandAgricultures, layerGroups, searchValue } = usePage().props;
    const perpage = useRef(typeLandAgricultures.per_page);
    const [isLoading, setIsLoading] = useState(false);
    const [search, setSearch] = useState('');
    const [openModal, setOpenModal] = useState(false);
    const [selectedTypeLandAgriculture, setSelectedTypeLandAgriculture] = useState(null);
    ...
    const getData = (isSearchCleared = false) => {
        ...
        router.get(
            route('jenis-lahan-pertanian.index'),
            params,
            {
                preserveScroll: true,
                preserveState: true,
    ...
    return (
        <BackpageLayout>
            <Head title="Master Data Jenis Pertanian" />
            <div className="min-h-[84dvh] rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default sm:px-7.5 xl:pb-1">
                <div className="flex justify-between gap-2 sm:gap-10">
                    ...
                    <button onClick={() => {
                        setOpenModal(true);
                        setSelectedTypeLandAgriculture(null);
                        setIsUpdate(false);
                    ...
                    <ModalInputTypeLandAgriculture
                        openModal={openModal}
                        setOpenModal={setOpenModal}
                        typeLandAgricultureById={selectedTypeLandAgriculture}
                ...
                <div className="flex flex-col my-2">
                    <Table striped>
                        ...
                        <Table.Body className="divide-y">
                            {!isLoading && typeLandAgricultures.data.map((typeLandAgriculture, index) => (
                                <Table.Row key={index} className="bg-white">
                                    <Table.Cell className="whitespace-nowrap font-medium text-gray-900 w-5">
                                        {(typeLandAgricultures.current_page - 1) * typeLandAgricultures.per_page + index + 1}
                                    </Table.Cell>
                                    <Table.Cell className="whitespace-nowrap font-medium text-gray-900">
                                        {typeLandAgriculture.name}
                                    </Table.Cell>
                                    <Table.Cell className='flex justify-center items-center gap-3'>
                                        <button
                                            onClick={() => handleEditClick(typeLandAgriculture)}
                                        >
                                            <i className="mr-2 fa-solid fa-pen-to-square text-blue-500 fa-xl"></i>
                                        </button>
                                        <button
                                            onClick={() => deteleDataConfirm(typeLandAgriculture.id)}
                                        >
                    ...
                    {isLoading && <LoadData />}
                    {typeLandAgricultures.data.length <= 0 && !isLoading && <DataNotFound />}
                </div>
                {typeLandAgricultures.data.length > 0 && !isLoading &&
                    <div className="flex items-center justify-between m-2">
                        <div className="">
                            Showing {typeLandAgricultures.from} to {typeLandAgricultures.to} total{" "} {typeLandAgricultures.total}
                        </div>
