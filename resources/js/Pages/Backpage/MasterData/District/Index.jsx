...
import ModalInputDistrict from './ModalInput';

export default function MasterDataDistrictPage() {
    const { districts, searchValue } = usePage().props;
    const perpage = useRef(districts.per_page);
    ...
    const deleteData = async (id) => {
        await router.delete(route('kecamatan.destroy', { id: id }));
    };
    ...
    return (
        <BackpageLayout>
            <Head title="Master Data Kecamatan di Buleleng" />
            <div className="min-h-[84dvh] rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default sm:px-7.5 xl:pb-1">
                ...
                <div className="flex flex-col my-2">
                            ...
                            {!isLoading && districts.data.map((district, index) => (
                                <Table.Row key={index} className="bg-white">
                                    ...
                                    <Table.Cell className='flex justify-center items-center gap-3'>
                                        <Link href={`/master-data/kecamatan/${district.id}`}>
                    ...
                    {isLoading && <LoadData />}
                    {districts.data.length <= 0 && !isLoading && <DataNotFound />}
                </div>
