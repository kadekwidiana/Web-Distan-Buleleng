...
export default function MasterDataCommodityPage() {
    ...
    const perpage = useRef(commodities.per_page);
    const [isLoading, setIsLoading] = useState(false);
    const [search, setSearch] = useState('');
    const typeAgriculture = useRef(commodities.typeAgriculture);
            ...
            <div className="min-h-[84dvh] rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default sm:px-7.5 xl:pb-1">
                <div className="flex justify-between gap-2 sm:gap-10">
                    ...
                    <ModalInputCommodity
                        ...
                        commodityById={selectedCommodity}
                ...
                <div className="flex flex-col my-2 overflow-x-auto">
                        ...
                        <Table.Body className="divide-y">
                            {!isLoading && commodities.data.map((commodity, index) => (
                                <Table.Row key={index} className="bg-white">
                                    ...
                                    <Table.Cell className="whitespace-nowrap font-medium text-gray-900">
                                        {commodity.type_agriculture.name}
                                    </Table.Cell>
                                    <Table.Cell className='flex justify-center items-center gap-3'>
                                        ...
                                        <button
                                            onClick={() => deteleDataConfirm(commodity.id)}
                                        >
                    ...
                    {isLoading && <LoadData />}
                    {commodities.data.length <= 0 && !isLoading && <DataNotFound />}
                </div>
