...
import ModalInputUser from './ModalInput';

export default function ManagementUserPage() {
    const { persons, roleValue, searchValue } = usePage().props;
    const perpage = useRef(persons.per_page);
    const [isLoading, setIsLoading] = useState(false);
    const [search, setSearch] = useState('');
    const role = useRef(persons.role);
    ...
    const deleteData = async (id) => {
        await router.delete(route('data-pengguna.destroy', { id: id }));
    };
            ...
            <div className="min-h-[84dvh] rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default sm:px-7.5 xl:pb-1">
                <div className="flex justify-between gap-2 sm:gap-6">
                    <div className="flex w-full">
                        ...
                        <select defaultValue={roleValue} onChange={handleFilter} name='role' id="role" className="flex-shrink-0 z-10 hidden lg:inline-flex items-center py-2.5 text-sm font-medium text-center text-gray-900 bg-gray-100 border border-gray-300 hover:bg-gray-200 focus:ring-0 focus:ring-blue-500 focus:border-bluering-blue-500 focus:outline-none focus-visible:outline-none">
                ...
                <div className="flex flex-col my-2 overflow-x-auto">
                                ...
                                <Table.Row key={index} className="bg-white">
                                    ...
                                    {person.nik !== '0000000000000000'
                                        ...
                                        <Table.Cell className='flex justify-center items-center gap-3'>
                                            ...
                                            <button
                                                onClick={() => deteleDataConfirm(person.id)}
                                            >
                    ...
                    {isLoading && <LoadData />}
                    {persons.data.length <= 0 && !isLoading && <DataNotFound />}
                </div>
